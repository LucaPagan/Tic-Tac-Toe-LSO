...
\begin{titlepage}
    ...
    {\normalsize \textbf{Repository GitHub:}\\ \url{https://github.com/LucaPagan/Tic-Tac-Toe-LSO.git}}
...
Il ciclo di vita del software è gestito attraverso rigorose pratiche di versionamento e automazione:
\begin{itemize}
    \item \textbf{Versionamento}: Il progetto è gestito e tracciato tramite il sistema di controllo di versione \textbf{Git}.
...
Il thread dedicato al client legge ciclicamente il socket. Quando riceve \texttt{MAKE\_MOVE}, per esempio, aggiorna la matrice del gioco, verifica le condizioni di vittoria, e invia un broadcast (protetto da mutex) ai due socket coinvolti nella partita per aggiornare l'interfaccia.
...
L'architettura proposta garantisce stabilità, isolamento delle risorse e una gestione efficiente della memoria grazie alla corretta deallocazione e al riciclo degli identificatori logici.
